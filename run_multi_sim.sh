...
do
    ...
    for problem_str in ${shuffled}; do
        ...
        c="CUDA_VISIBLE_DEVICES=$gpu nice -n 19 python multi_batch_sim.py --problem_str=$problem_str --noisy=False --init_seed=$i --kernel=default --comp_noise_type=$cnt --device=$gpu"
        echo $c
        eval $c
    done
done
